...
<div class="w-full px-4 md:px-8 pb-10">
	<div
		...
		<section class="flex-1 flex flex-col bg-black/5 relative">
			<div id="chat-box"
			...
			<div id="pwo-rec-panel"
				class="hidden bg-emerald-50 p-3 border-t border-emerald-100 flex flex-col items-center">
				<canvas id="pwo-waveform" width="300" height="40"
					class="w-full h-10 mb-1"></canvas>
				<span id="pwo-timer"
			...
			<footer class="p-6 border-t border-white/5 bg-black/20">
				<div
					...
					<textarea id="chat-in" rows="1"
						...
						class="flex-1 bg-transparent border-none outline-none text-sm text-white py-3 resize-none leading-tight"></textarea>
					...
					<div class="flex items-center gap-3 pr-1 flex-shrink-0">
						<button id="pwo-mic"
							...
							<svg class="w-5 h-5" fill="none" stroke="currentColor"
                                ...
                                <path stroke-linecap="round"
									stroke-linejoin="round" stroke-width="2"
									d="M19 10v1a7 7 0 01-14 0v-1m7 11v-4m-3 4h6"></path>
                            </svg>
						</button>
...
<div id="pwo-lightbox"
	class="hidden fixed inset-0 bg-black/80 backdrop-blur-sm z-[9999] flex flex-col items-center justify-center p-4">
	<div class="absolute top-5 right-5 flex gap-4">
		<a id="pwo-lightbox-download" href="" download
			class="text-white bg-white/10 hover:bg-white/20 p-2 rounded-full transition-colors"><svg
				class="w-6 h-6" fill="none" stroke="currentColor"
				viewBox="0 0 24 24">
				<path
					d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"
					stroke-width="2"></path></svg></a>
		<button id="pwo-lightbox-close"
			class="text-white bg-white/10 hover:bg-white/20 p-2 rounded-full transition-colors">
			<svg class="w-6 h-6" fill="none" stroke="currentColor"
				viewBox="0 0 24 24">
				<path d="M6 18L18 6M6 6l12 12" stroke-width="2"></path></svg>
		</button>
	</div>
	<img id="pwo-lightbox-img" src=""
		class="max-w-full max-h-[85vh] rounded-lg shadow-2xl object-contain cursor-zoom-out">
</div>

<div id="pdf-modal"
	class="hidden fixed inset-0 bg-black/80 backdrop-blur-sm z-[9999] flex flex-col items-center justify-center p-4">
	<div
		class="w-full max-w-5xl h-[90vh] bg-white rounded-2xl overflow-hidden flex flex-col">
		<div class="flex justify-between items-center p-4 border-b">
			<h3 class="font-bold text-gray-700">PDF Viewer</h3>
			<button
				onclick="document.getElementById('pdf-modal').classList.add('hidden')"
...
<div id="video-modal"
	class="hidden fixed inset-0 bg-black/90 backdrop-blur-md z-[10000] flex items-center justify-center p-4">
	<button onclick="window.closeVideoModal()"
		class="absolute top-6 right-6 text-white/70 hover:text-white text-4xl">&times;</button>
	<div
		class="w-full max-w-5xl h-auto max-h-[85vh] flex items-center justify-center">
