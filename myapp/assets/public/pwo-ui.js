...
export function render(data, isNew = true, isTemp = false) {
    ...
    const deleteBtn = (data.is_me && data.id) ? `
        <button class="pwo-delete-btn absolute -left-6 top-1/2 -translate-y-1/2 text-gray-300 hover:text-red-500 transition-colors" data-id="${data.id}" title="Delete Message">
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
        </button>
    ` : '';
