...
/**
 * Modernized S3 Class (PHP 7.4 - 8.3 compatible)
 * Fully Typed - No functions removed.
 */
final class S3
{
    // ACL flags
    ...
    public static function listBuckets(bool $detailed = false)
    {
        ...
        if ($detailed) {
            if (isset($rest->body->Owner, $rest->body->Owner->ID, $rest->body->Owner->DisplayName))
                $results['owner'] = ['id' => (string)$rest->body->Owner->ID, 'name' => (string)$rest->body->Owner->DisplayName];
            $results['buckets'] = [];
            foreach ($rest->body->Buckets->Bucket as $b)
                $results['buckets'][] = ['name' => (string)$b->Name, 'time' => strtotime((string)$b->CreationDate)];
        } else {
    ...
    public static function getBucket(string $bucket, ?string $prefix = null, ?string $marker = null, ?int $maxKeys = null, ?string $delimiter = null, bool $returnCommonPrefixes = false)
    {
        ...
        if ($maxKeys !== null) $rest->setParameter('max-keys', (string)$maxKeys);
    ...
    public static function inputResource(&$resource, $bufferSize = false, string $md5sum = '')
    {
        ...
        return ['size' => $bufferSize, 'md5sum' => $md5sum, 'fp' => &$resource];
    }
    ...
    public static function putObject($input, string $bucket, string $uri, string $acl = self::ACL_PRIVATE, array $metaHeaders = [], array $requestHeaders = [], string $storageClass = self::STORAGE_CLASS_STANDARD, string $serverSideEncryption = self::SSE_NONE): bool
    {
        ...
        foreach ($requestHeaders as $h => $v) strpos($h, 'x-amz-') === 0 ? $rest->setAmzHeader($h, $v) : $rest->setHeader($h, $v);
        
        $rest->setAmzHeader('x-amz-acl', $acl);
        if ($storageClass !== self::STORAGE_CLASS_STANDARD) $rest->setAmzHeader('x-amz-storage-class', $storageClass);
        if ($serverSideEncryption !== self::SSE_NONE) $rest->setAmzHeader('x-amz-server-side-encryption', $serverSideEncryption);
    ...
    public static function getAuthenticatedURL(string $bucket, string $uri, int $lifetime, bool $hostBucket = false, bool $https = false): string
    {
        ...
        return sprintf(($https ? 'https' : 'http') . '://%s/%s?AWSAccessKeyId=%s&Expires=%u&Signature=%s', 
            $host, $uri, self::$__accessKey, $expires, $sig);
    }
    ...
    private static function __getCloudFrontDistributionConfigXML(string $bucket, bool $enabled, string $comment, string $callerReference, array $cnames): string
    {
        ...
        foreach ($cnames as $c) $conf->appendChild($dom->createElement('CNAME', $c));
        if ($comment !== '') $conf->appendChild($dom->createElement('Comment', $comment));
    ...
    {
        if (isset($node->DistributionConfig)) return self::__parseCloudFrontDistributionConfig($node->DistributionConfig);
        return [
            'id' => (string)$node->Id,
    ...
    public static function __getSignatureV4(array $amzHeaders, array $headers, string $method, string $uri, array $parameters): string
    {
        ...
        $combinedHeaders = [];
        foreach (array_merge($headers, $amzHeaders) as $k => $v) $combinedHeaders[strtolower($k)] = trim((string)$v);
        ...
        $canonicalHeaders = '';
        foreach ($combinedHeaders as $k => $v) $canonicalHeaders .= $k . ':' . $v . "\n";
        ...
        $stringToSign = implode("\n", [$algorithm, $amzHeaders['x-amz-date'], $scope, hash('sha256', $canonicalRequest)]);
...
final class S3Request
{
    ...
    private array $headers = ['Host' => '', 'Date' => '', 'Content-MD5' => '', 'Content-Type' => ''];
    public $fp = null;
    public int $size = 0;
    public $data = false;
    ...
    public function setParameter(string $key, ?string $value): void { $this->parameters[$key] = $value; }
    public function setHeader(string $key, string $value): void { $this->headers[$key] = $value; }
    public function setAmzHeader(string $key, string $value): void { $this->amzHeaders[$key] = $value; }
    ...
    public function getResponse(): stdClass
    {
        ...
        if (S3::hasAuth()) {
            ...
            foreach ($this->amzHeaders as $h => $v) $httpHeaders[] = "$h: $v";
            foreach ($this->headers as $h => $v) if (!empty($v)) $httpHeaders[] = "$h: $v";
        ...
        if (is_resource($this->fp)) {
            if ($this->verb === 'PUT') curl_setopt($curl, CURLOPT_UPLOAD, true);
    ...
    { 
        if (is_resource($this->fp)) return (int)fwrite($this->fp, $data);
        $this->response->body .= $data; return strlen($data); 
    }

    public function __responseHeaderCallback(\CurlHandle $curl, string $data): int 
    { 
        $parts = explode(': ', $data, 2);
        if (count($parts) === 2) $this->response->headers[strtolower($parts[0])] = trim($parts[1]);
        return strlen($data); 
    }
}
