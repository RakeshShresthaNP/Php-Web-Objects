<div class="w-full px-4 md:px-8 pb-10">
    ...
    <div class="mb-8 flex flex-col gap-1">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <h2 class="text-3xl font-black tracking-tight text-white">Users</h2>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-black/20 border-b border-white/5 text-[10px] uppercase font-bold text-gray-500 tracking-widest">
                        <th class="px-6 py-5">Username (Email)</th>
                        <th class="px-6 py-5">Name</th>
                        <th class="px-6 py-5 text-center">Registered Date</th>
                ...
                <tbody class="divide-y divide-white/5">
                    ...
                    <tr class="hover:bg-white/[0.02] transition-colors group">
                        ...
                        <td class="px-6 py-4 text-center text-sm text-gray-500">
                            <?php echo date('F d, Y', strtotime($user->d_created)); ?>
                        </td>
                        ...
                        <td class="px-6 py-4 text-right whitespace-nowrap">
                            <div class="flex justify-end gap-2">
                                <a href="<?php echo getUrl('manage/users/edit/'.$user->id) ?>" class="p-2 bg-white/5 hover:bg-blue-500/10 rounded-lg text-gray-500 hover:text-[#4d7cfe] transition-all" title="Edit">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                </a>
                                <a href="<?php echo getUrl('manage/users/'.($user->status == 1 ? 'disable' : 'enable').'/'.$user->id) ?>" class="p-2 bg-white/5 rounded-lg transition-all <?php echo $user->status == 1 ? 'text-amber-500/50 hover:text-amber-500' : 'text-emerald-500/50 hover:text-emerald-500'; ?>" title="Toggle">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>
                                </a>
                                <a href="<?php echo getUrl('manage/users/delete/'.$user->id) ?>" onclick="return confirm('Delete permanently?');" class="p-2 bg-red-500/5 hover:bg-red-500/20 rounded-lg text-red-500/40 hover:text-red-500 transition-all" title="Delete">
                    ...
                    <?php endforeach; else: ?>
                    <tr><td colspan="5" class="px-6 py-12 text-center text-gray-600 italic">No user records found.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
        <?php if (!empty($links)): ?>
            <div class="px-6 py-5 border-t border-white/5 bg-black/10 flex justify-center"><?php echo $links; ?></div>
        <?php endif; ?>
    </div>

    <div class="mb-6 flex flex-col gap-1">
        <h3 class="text-xl font-bold text-white pl-1">Partner Configuration</h3>
        <p class="text-[10px] text-gray-500 pl-1 uppercase tracking-wider font-bold">Manage Integrated Partners</p>
    </div>
    ...
    <div class="w-full bg-[#151a21] border border-white/5 rounded-2xl shadow-xl overflow-hidden">
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-black/20 border-b border-white/5 text-[10px] uppercase font-bold text-gray-500 tracking-widest">
                    <th class="px-6 py-5 w-20 text-center">ID</th>
                    <th class="px-6 py-5">Partner Name</th>
                    <th class="px-6 py-5">Hostname</th>
                    <th class="px-6 py-5">Email Address</th>
                    <th class="px-6 py-5 text-right">Settings</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-white/5">
                            ...
                            <div class="flex flex-col gap-1 items-end">
                                <?php foreach ($p->settings as $set): ?>
                                    <div class="flex items-center gap-2 text-[10px] text-gray-400 bg-white/5 px-2 py-1 rounded border border-white/5">
                                        <span>Host: <span class="text-gray-200 font-mono"><?php echo $set->mailhost ?></span></span>
                                    </div>
                                <?php endforeach; ?>
                            </div>
                        <?php else: ?><span class="text-xs italic text-gray-600">No settings</span><?php endif; ?>
                    </td>
                </tr>
                <?php endforeach; else: ?>
                <tr><td colspan="5" class="px-6 py-12 text-center text-gray-600 italic">No partners found.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
